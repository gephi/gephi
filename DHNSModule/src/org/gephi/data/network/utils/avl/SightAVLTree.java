...
/**
 * Copy of {@link SimpleAVLTree} but with a {@link Sight} item type. The <b><code>ID</code></b> of
 * <code>Sight</code> is used as key. An additional item <code>enabled</code> has been set in tree
 * nodes. Specific method <code>isEnabled</code> has been added for retrieving the <code>enabled</code>
 * flag associated to the <code>Sight</code>. The <code>setEnabled</code> method set the flag.
 *
...
public class SightAVLTree implements Iterable<SightImpl> {
    ...
    public boolean add(SightImpl item, boolean enabled) {
        ...
        if (p == null) {
            this.root = new SightAVLNode(item, enabled);
        } else {
            while (true) {
                ...
                if (c < 0) {
                    if (p.left != null) {
                        ...
                        p.left = new SightAVLNode(item, p, enabled);
                        p.balance--;
                    ...
                    if (p.right != null) {
                        ...
                        p.right = new SightAVLNode(item, p, enabled);
                        p.balance++;
    ...
    public boolean remove(SightImpl item) {
        ...
        while (p != null) {
            ...
            if (c < 0) {
                ...
                SightAVLNode y; // node from which rebalancing begins
    ...
    public boolean isEnabled(SightImpl sight) {
        ...
        while (p != null) {
            int c = sight.getNumber() - p.item.getNumber();
    ...
    public SightAVLIterator iterator() {
        return new SightAVLIterator(this);
    }

    public SightAVLIterator freeIterator() {
        return new SightAVLIterator();
    }
    ...
    /**
     * Iterator for the {@link SightAVLTree}. With the default constructor the Iterator can exist as
    ...
    public class SightAVLIterator implements Iterator<SightImpl>, ResetableIterator {
        ...
        public SightAVLIterator(SightAVLNode node) {
            this.next = node;
            goToDownLeft();
        }

        public SightAVLIterator(SightAVLTree tree) {
            this(tree.root);
        }

        public void setNode(SightAVLTree tree) {
            this.next = tree.root;
            goToDownLeft();
        }
