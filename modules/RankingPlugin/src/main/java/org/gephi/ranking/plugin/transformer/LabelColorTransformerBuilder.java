...
/**
 * Label color transformer builder. Builds
 * <code>LabelColorTransformer</code> instances, that receives
 * {@link Renderable} targets.
 *
...
public class LabelColorTransformerBuilder implements TransformerBuilder {
    ...
    public static class LabelColorTransformer extends AbstractColorTransformer<Element> {

        @Override
        public Object transform(Element target, float normalizedValue) {
            Color color = getColor(normalizedValue);
            target.getTextProperties().setColor(color);
            return color;
        }
    }
}
