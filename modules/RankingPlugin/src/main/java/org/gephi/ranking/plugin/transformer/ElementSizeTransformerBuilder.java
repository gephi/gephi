...
/**
 * Renderable size transformer builder. Builds
 * <code>RenderableSizeTransformer</code> instances, that receives
...
public class ElementSizeTransformerBuilder implements TransformerBuilder {
    ...
    public static class RenderableSizeTransformer extends AbstractSizeTransformer<Element> {

        @Override
        public Object transform(Element target, float normalizedValue) {
            float size = getSize(normalizedValue);
            if (target instanceof Node) {
                ((Node) target).setSize(size);
            } else if (target instanceof Edge) {
                ((Edge) target).setWeight(size);
            }
