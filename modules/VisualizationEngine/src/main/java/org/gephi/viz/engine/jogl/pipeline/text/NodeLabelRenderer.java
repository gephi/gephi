...
public class NodeLabelRenderer implements Renderer<JOGLRenderingTarget, VoidWorldData> {
    ...
    private final LabelAtlasCache cache =
        new LabelAtlasCache( /*maxEntries*/ 4096, /*pageW*/ 2048, /*pageH*/ 2048, /*padding*/ 1);
    ...
    private static final float RASTER_SCALE = 0.6f;
    // render at 60% of logical px, reduce for less crisp text with smaller textures
    ...
    public void render(VoidWorldData data, JOGLRenderingTarget target, RenderingLayer layer) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (pxHeight <= 0 || !isFinite(nodeDiameterPx)) {
                continue;
            }
            ...
            final LabelSprite sprite =
                cache.getOrCreate(gl, fontCollection, paragraphStyle, text, pxHeight, argb, RASTER_SCALE, USE_MIPMAPS);
            if (sprite == null) {
                continue;
            }
            ...
            bv.putQuad(x0n, y0n, u0, v1, x0n, y1n, u0, v0, x1n, y1n, u1, v0, x0n, y0n, u0, v1, x1n, y1n, u1, v0, x1n,
                y0n, u1, v1);
        }
    ...
    private static final String VERT_SRC = "attribute vec2 aPos;" + "attribute vec2 aUV;" + "varying vec2 vUV;" +
    ...
    private static final class AtlasPage {
        ...
        AtlasPage(GL2ES2 gl, int w, int h, boolean mipmaps) {
            ...
            gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER,
                mipmaps ? GL.GL_LINEAR_MIPMAP_LINEAR : GL.GL_LINEAR);
        ...
        boolean canFit(int needW, int needH) {
            ...
            // try new shelf
            if (curY + shelfH + needH <= h && needW <= w) {
                return true;
            }
            return false;
        }
        ...
        Placement alloc(int needW, int needH) {
            ...
            shelfH = 0;
            if (!canFit(needW, needH)) {
                return null; // double-check after moving
            }
            Placement p = new Placement(curX, curY, needW, needH);
            curX += needW;
            shelfH = Math.max(shelfH, needH);
