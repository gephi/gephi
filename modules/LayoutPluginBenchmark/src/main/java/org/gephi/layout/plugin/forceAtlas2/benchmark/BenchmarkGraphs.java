...
final class BenchmarkGraphs {

    private BenchmarkGraphs() {
    }

    static GraphModel createRandomUndirectedGraph(int nodeCount, double edgeProbability, long seed) {
        ...
        for (int i = 0; i < nodeCount - 1; i++) {
            for (int j = i + 1; j < nodeCount; j++) {
                if (random.nextDouble() < edgeProbability) {
                    Edge edge = graphModel.factory().newEdge(nodes[i], nodes[j], false);
                    graph.addEdge(edge);
                }
            }
        }
