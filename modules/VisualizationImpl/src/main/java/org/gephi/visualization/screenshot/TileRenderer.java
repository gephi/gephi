...
/**
 * Note: Code copied from JOGL 2.0.2 sources
 * http://jogamp.org/deployment/maven/org/jogamp/jogl/jogl/2.0.2/ This class is
 * no longer included since JOGL 2.1.0 and up (no clear reason why).
 *
 * We keep this class here until a better solution is found, or TileRenderer is
 * included again in a new JOGL release.
 */
/**
 * A fairly direct port of Brian Paul's tile rendering library, found at
 * <a href = "http://www.mesa3d.org/brianp/TR.html">
 * http://www.mesa3d.org/brianp/TR.html </a> . I've java-fied it, but the
 * functionality is the same.
 *
 * Original code Copyright (C) 1997-2005 Brian Paul. Licensed under
 * BSD-compatible terms with permission of the author. See LICENSE.txt for
 * license information.
 *
 * @author ryanm
 */
public class TileRenderer {
  ...
  //
    ...
    /**
     * Sets the size of the tiles to use in rendering. The actual effective size
     * of the tile depends on the border size, ie ( width - 2*border ) * (
     * height - 2 * border )
     *
     * @param width The width of the tiles. Must not be larger than the GL
     * context
     * @param height The height of the tiles. Must not be larger than the GL
     * context
     * @param border The width of the borders on each tile. This is needed to
     * avoid artifacts when rendering lines or points with thickness > 1.
     */
    ...
    /**
     * Specify a buffer the tiles to be copied to. This is not necessary for the
     * creation of the final image, but useful if you want to inspect each tile
     * in turn.
     *
     * @param format Interpreted as in glReadPixels
     * @param type Interpreted as in glReadPixels
     * @param image The buffer itself. Must be large enough to contain a tile,
     * minus any borders
     */
     ...
     * @param format Interpreted as in glReadPixels
     * @param type Interpreted as in glReadPixels
     * @param image the buffer itself, must be large enough to hold the final
     * image
     */
     ...
     *
     * @param order The row traversal order, must be eitherTR_TOP_TO_BOTTOM or
     * TR_BOTTOM_TO_TOP
     */
    ...
    /**
     * Sets the context to use an orthographic projection. Must be called before
     * rendering the first tile
     *
     * @param left As in glOrtho
     * @param right As in glOrtho
     * @param bottom As in glOrtho
     * @param top As in glOrtho
     * @param zNear As in glOrtho
     * @param zFar As in glOrtho
     */
    ...
    /**
     * Sets the perspective projection frustrum. Must be called before rendering
     * the first tile
     *
     * @param left As in glFrustrum
     * @param right As in glFrustrum
     * @param bottom As in glFrustrum
     * @param top As in glFrustrum
     * @param zNear As in glFrustrum
     * @param zFar As in glFrustrum
     */
    ...
    /**
     * Begins rendering a tile. The projection matrix stack should be left alone
     * after calling this
     *
     ...
     *
     * @param gl the gl context
     * @return true if there are more tiles to be rendered, false if the final
     * image is complete
     */
    ...
    /**
     * Tile rendering causes problems with using glRasterPos3f, so you should
