...
- **`release:X` (including `release:latest`) means "has at least one event tagged with this release," not "introduced by this release."** A release that shipped a week ago will still pull in issues that are months or a year old, because anyone still on that version hitting an old bug counts. This is correct for the skill's purpose (surfacing the current backlog) but must not be conflated with regressions new to the release — see Step 1 for the required follow-up query that separates the two.
...
**Required follow-up — split new-since-release from pre-existing.** Because `release:latest` matches on "tagged with this release," not "first seen in this release" (see Inputs), immediately run a second `search_issues` query adding `firstSeen:-Nd` to the query string, where N is the number of days since the target release shipped (e.g. `firstSeen:-7d` for a release one week old). The count this returns is the number of issues genuinely new since the release; the difference from the full backlog count is pre-existing issues still being hit on the current version. Both counts must be stated explicitly and prominently in the report's Summary (Step 6), not left implicit in a table — a report that doesn't separate them reads as far more alarming than the actual regression count warrants.

**Don't trust the list view's "First seen" field for age claims.** `search_issues`' list output under-reports first-seen for any issue older than the query's `period`/lookback window — the date clusters near the window boundary instead of showing the true origin. It's only reliable for issues that are genuinely recent (within the window), where no distortion is possible. To confirm a specific issue's true age, use `get_sentry_resource`'s per-issue detail view, which reports the correct first-seen date. Don't call `get_sentry_resource` on every issue just to bucket old vs. new, though — the `firstSeen:-Nd` query above already does that for the whole backlog in one call.
...
- Of the N unresolved issues, only M are new since this release (`firstSeen:-Nd`, N=<days since release shipped>); the rest are pre-existing bugs still being hit on the current version
